/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   specifier1.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cliew <cliew@student.42singapore.sg>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/09 17:03:07 by cliew             #+#    #+#             */
/*   Updated: 2023/10/16 23:27:33 by cliew            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	repeat_char(char char_to_repeat, int repeat_time, int *count)
{
	int	i;

	i = 0;
	if (repeat_time < 0)
		return (-1);
	// printf("repeat time is %d",repeat_time);
	while (i < repeat_time)
	{
		*count += write(1, &char_to_repeat, sizeof(char_to_repeat));
		i++;
	}
	return (repeat_time);
}
int	putchar_output(char c, int *count)
{
	*count += write(1, &c, 1);
	return (1);
}

int	specifier_c(int flags[][5], int arg, int *count)
{
	int	width;
	int	len;

	len = 1;
	width = 0;
	if (flags[1][0] == 1)
		width = flags[1][1];
	if (flags[0][0] == 1 || ((width < len) && (width != 0)))
	{
		// printf("trigger first");
		*count += write(1, &arg, 1);
	}
	if (width > len)
	{
		repeat_char(' ', width - len, count);
	}
	if (flags[0][0] == 0)
	{
		*count += write(1, &arg, 1);
		// printf("trigger second");
	}
	return (1);
}

int	specifier_s(int flags[][5], char *args, int *count)
{
	int		width;
	int		len;
	char	*arg;

	arg = (char *)args;
	if (!arg && flags[2][0] == 1 && flags[2][1] < 6)
		arg = "";
	else if (!arg)
		arg = "(null)";
	if (arg && flags[2][0] == 1 && flags[2][1] < (int)(ft_strlen(arg)))
		len = flags[2][1];
	else if (arg)
		len = ft_strlen(arg);
	width = len;
	if (flags[1][0] == 1)
		width = flags[1][1];
	if (flags[0][0] == 1 && arg)
		ft_putstr(arg, len, count);
	repeat_char(' ', (width - len), count);
	if (flags[0][0] == 0 && arg)
		ft_putstr(arg, len, count);
	return (len);
}

int	put_sign(int *count, int flags[][5], ...)
{
	va_list			args;
	int				integer;
	unsigned int	un_integer;

	va_start(args, flags);
	integer = va_arg(args, int);
	un_integer = va_arg(args, unsigned int);
	if (integer || integer == 0)
	{
		if (flags[0][1] == 1 && integer >= 0)
			putchar_output('+', count);
		else if (integer < 0)
			putchar_output('-', count);
		else if (flags[0][2] == 1)
			putchar_output(' ', count);
	}
	else if (un_integer && flags[0][2] == 1)
		putchar_output(' ', count);
	return (1);
}

int	check_len(int flags[][5], int arg, int p_arg)
{
	int		len;
	int		p_len;
	char	*str;
	char	*p_str;

	str = ft_itoa(arg);
	len = ft_strlen(str);
	p_str = ft_itoa(p_arg);
	p_len = ft_strlen(p_str);
	if (flags[2][0] == 1 && flags[2][1] > p_len)
	{
		len = flags[2][1];
		if (arg < 0)
			len = len + 1;
	}
	if ((flags[0][1] == 1 || flags[0][2] == 1) && arg >= 0)
		len = len + 1;
	free(str);
	free(p_str);

	return (len);
}

int	check_ulen(int flags[][5], unsigned int arg)
{
	int		len;
	char	*str;

	str = ft_itoa(arg);
	len = ft_strlen(str);
	if (((int)arg) < 0)
		len = len - 1;
	if (flags[2][0] == 1 && flags[2][1] > len)
		len = flags[2][1];
	free(str);
	return (len);
}

int	check_filler(int flags[][5])
{
	if (flags[0][4] == 1)
		if (flags[2][0] == 0)
			return ('0');
		else
			return (' ');
	else if (flags[0][4] == 0)
		return (' ');
	return (1);
}

int	handle_negative(int arg)
{
	long	pos;

	pos = arg;
	if (arg == -2147483648)
		return (2147483648);
	else if (arg < 0)
	{
		pos = pos * -1;
		return (pos);
	}
	else
		return (pos);
}

int	check_width(int flags[][5])
{
	int	width;

	if (flags[1][0] == 1)
		width = flags[1][1];
	else
		width = 0;
	return (width);
}

int	d_logic(int flags[][5], int width, int arg, char filler, int *count)
{
	char	*str;
	int		p_int;
	int		len;

	p_int = handle_negative(arg);
	str = ft_itoa(p_int);
	len = check_len(flags, arg, p_int);
	if (flags[0][0] != 1 && filler == ' ')
		repeat_char(filler, width - len, count);
	put_sign(count, flags, arg);
	if (flags[0][0] != 1 && filler == '0')
		repeat_char(filler, width - len, count);
	if (flags[2][0] == 1 && flags[2][1] > (int)(ft_strlen(str)))
		repeat_char('0', flags[2][1] - (int)(ft_strlen(str)), count);
	ft_putuint(p_int, count, flags);
	if (flags[0][0] == 1)
		repeat_char(' ', width - len, count);
	free(str);
	return (len);
}

int	specifier_d(int flags[][5], int arg, int *count)
{
	int		width;
	int		len;
	char	filler;

	filler = check_filler(flags);
	// printf("len is %d",len);
	width = check_width(flags);
	
	len = d_logic(flags, width, arg, filler, count);
	return (len);
}

// int	specifier_d(int flags[][5], int arg, int *count)
// {
// 	int		width;
// 	int		len;
// 	char	filler;
// 	char	*str;
// 	int		p_int;

// 	p_int = handle_negative(arg);
// 	str = ft_itoa(p_int);
// 	filler = check_filler(flags);
// 	len = check_len(flags, arg);
// 	// printf("len is %d",len);
// 	width = 0;
// 	if (flags[1][0] == 1)
// 		width = flags[1][1];
// 	if (flags[0][0] != 1 && filler==' ')
// 		repeat_char(filler, width - len, count);
// 	put_sign(count, flags, arg);
// 	if (flags[0][0] != 1 && filler=='0')
// 		repeat_char(filler, width - len, count);
// 	if (flags[2][0] == 1 && flags[2][1] > (int)(ft_strlen(str)))
// 		repeat_char('0', flags[2][1] - (int)(ft_strlen(str)), count);

// 	ft_putuint(p_int, count,flags);
// 	if (flags[0][0] == 1)
// 		repeat_char(' ', width - len, count);
// 	free(str);
// 	return (len);
// }

// int	specifier_i(int flags[][5], int arg)
// {
// 	int		width;
// 	int		len;
// 	char	filler;

// 	width = 0;
// 	len = check_len(flags, arg);
// 	filler = check_filler(flags);
// 	if (flags[1][0] == 1)
// 		width = flags[1][1];
// 	if (flags[0][0] != 1)
// 	{
// 		if (width > len)
// 			repeat_char(filler, width - len);
// 	}
// 	put_sign(flags, arg);
// 	if (flags[2][0] == 1 && flags[2][1] > (int)(ft_strlen(ft_itoa(arg))))
// 		repeat_char('0', flags[2][1] - (int)(ft_strlen(ft_itoa(arg))));
// 	ft_putint(arg);
// 	if (flags[0][0] == 1)
// 	{
// 		if (width > len)
// 			repeat_char(' ', width - len);
// 	}
// 	return (1);

// }

// int	specifier_u(int flags[][5], unsigned int arg, int *count)
// {
// 	int				width;
// 	int				len;
// 	char			filler;
// 	char			*str;
// 	unsigned int	p_int;

// 	width = 0;
// 	len = check_ulen(flags, arg);
// 	p_int = handle_negative(arg);
// 	str = ft_itoa(p_int);
// 	filler = check_filler(flags);
// 	if (flags[1][0] == 1)
// 		width = flags[1][1];
// 	if (flags[0][0] != 1)
// 	{
// 		if (width > len)
// 			repeat_char(filler, width - len, count);
// 	}
// 	if (flags[2][0] == 1 && flags[2][1] > (int)ft_strlen(str))
// 		repeat_char('0', flags[2][1] - (int)ft_strlen(str), count);
// 	ft_putuint(arg, count, flags);
// 	if (flags[0][0] == 1)
// 	{
// 		if (width > len)
// 			repeat_char(' ', width - len, count);
// 	}
// 	free(str);
// 	return (1);
// }
int	u_logic(int flags[][5], int width, int arg, char filler, int *count)
{
	char	*str;
	int		p_int;
	int		len;

	p_int = handle_negative(arg);
	str = ft_itoa(p_int);
	len = check_len(flags, arg, p_int);
	if (flags[0][0] != 1 && filler == ' ')
		repeat_char(filler, width - len, count);
	put_sign(count, flags, arg);
	if (flags[0][0] != 1 && filler == '0')
		repeat_char(filler, width - len, count);
	if (flags[2][0] == 1 && flags[2][1] > (int)(ft_strlen(str)))
		repeat_char('0', flags[2][1] - (int)(ft_strlen(str)), count);
	ft_putuint(p_int, count, flags);
	if (flags[0][0] == 1)
		repeat_char(' ', width - len, count);
	free(str);
	return (len);
}

int	specifier_u(int flags[][5], unsigned int arg, int *count)
{

	int		width;
	int		len;
	char	filler;

	filler = check_filler(flags);
	// printf("len is %d",len);
	width = check_width(flags);
	
	len = d_logic(flags, width, arg, filler, count);
	return (len);
}