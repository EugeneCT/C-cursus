...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/22 14:24:18 by cliew             #+#    #+#             */
/*   Updated: 2023/12/22 14:24:18 by cliew            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_file(int fd, struct s_fileInfo *fd_i, char **buffer, char **n_b)
{
	...
	while (b_count >= 0 && eol == 0)
	{
		...
		ft_free(n_b, 0, 0);
		*n_b = ft_strncpy(*buffer, b_count, 0);
...
char	*get_result(int fd, struct s_fileInfo *fd_i)
{
	...
	{
		fd_i[fd].c_line = ft_strncpy(fd_i[fd].p_line, 1, 1);
...
char	*get_next_line(int fd)
{
	...
	if (fd_i[fd].p_line && ft_strchr(fd_i[fd].p_line, '\n'))
	{
		str = ft_strncpy(fd_i[fd].p_line, 1, 1);
		ft_strchar_mod(str, '\n', fd, fd_i);
		free(str);
		return (fd_i[fd].c_line);
	}
