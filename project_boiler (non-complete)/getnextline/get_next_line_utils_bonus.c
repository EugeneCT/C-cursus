...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/22 14:23:48 by cliew             #+#    #+#             */
/*   Updated: 2023/12/22 14:23:57 by cliew            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strncpy(char *src, size_t n, int full_cpy)
{
	size_t	i;
	size_t	len;
	char	*dest;
...
int	ft_strchar_mod(char *str, int chr, int fd, struct s_fileInfo *fd_i)
{
	...
	if (!fd_i[fd].c_line)
		fd_i[fd].c_line = ft_strncpy(str, end_current + 1, 0);
	else
	{
		temp_cur = ft_strncpy(str, end_current + 1, 0);
		temp_join = ft_strjoin(fd_i[fd].c_line, temp_cur);
	...
	if (i - (end_current + 1) > 0 && fd_i[fd].file_end != 1)
		fd_i[fd].p_line = ft_strncpy(str + end_current + 1, i - end_current, 0);
	else
		fd_i[fd].p_line = NULL;
	return (eol);
}
